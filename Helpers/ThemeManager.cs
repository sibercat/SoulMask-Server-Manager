...
public static class ThemeManager
{
    ...
    // ── Apply theme ───────────────────────────────────────────────────────────
    /// <summary>
    /// Sets the system-level WinForms color mode (handles button text, form
    /// backgrounds, input colors automatically) and then applies custom
    /// overrides for accent buttons, grids, console boxes, and the status bar.
    /// </summary>
    public static void Apply(Form form, AppTheme theme)
    {
        ...
        // 1. System-level dark / light mode — fixes button text, panel bg, etc.
        Application.SetColorMode(dark ? SystemColorMode.Dark : SystemColorMode.Classic);

        // 2. Windows 11 title bar
        SetTitleBarTheme(form.Handle, dark);

        // 3. Custom overrides on top of system theming
        ApplyCustom(form, dark);

        form.Refresh();
    }

    /// <summary>
    /// Called from the form's Shown event. Windows re-applies dark mode theming
    /// to child controls when the form is first shown, undoing any SetWindowTheme
    /// calls made in the constructor. This re-applies the override after that.
    /// </summary>
    ...
    // ── Custom per-control overrides ─────────────────────────────────────────
    // Must explicitly set all control colors — Application.SetColorMode only
    // applies to top-level windows, not child-embedded forms (TopLevel=false).
    private static void ApplyCustom(Control ctrl, bool dark)
    {
        switch (ctrl)
        {
            // ── Always-dark console output boxes ─────────────────────────────
            ...
            // ── StatusStrip — always accent-blue ─────────────────────────────
            case StatusStrip ss:
                ss.BackColor = Accent;
            ...
            // ── Labels (skip LinkLabels — they have explicit link colors set) ──
        ...
        // Set base form/control background
        if (ctrl is Form frm)
    ...
    private static void StyleTabControl(TabControl tc, bool dark)
    {
        ...
        tc.Padding    = tc.Padding; // force repaint

        // Remove before re-adding to avoid duplicate handlers on re-theme
        tc.DrawItem -= OnDrawTabItem;
        tc.DrawItem += OnDrawTabItem;

        // Paint the tab strip background (the area around/behind the tab headers)
    ...
    private static void OnPaintTabStrip(object? sender, PaintEventArgs e)
    {
        ...
        // Fill the entire tab strip area (top portion above the content area)
    ...
    // ── DataGridView — S3Lite-style dark grid ────────────────────────────────
    ...
    // ── ListView — S3Lite-style owner-draw ──────────────────────────────────
    private static void StyleListView(ListView lv, bool dark)
    {
        ...
        if (lv.OwnerDraw) return; // already set up
