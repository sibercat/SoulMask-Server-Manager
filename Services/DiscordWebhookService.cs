...
public class DiscordWebhookService
{
            ...
            var payload = new
            {
                embeds = new[]
                {
                    new
                    {
                        ...
                        footer = new { text = "SoulMask Server Manager" }
                    }
                }
            };
    ...
    public Task<bool> NotifyBackup(string url, string serverName, string backupFile) =>
        SendAsync(url, "💾 Backup Created", $"**{serverName}** backup saved: `{Path.GetFileName(backupFile)}`", 0x9C6BC9);

    public Task<bool> TestAsync(string url) =>
        SendAsync(url, "🔔 Test Notification", "SoulMask Server Manager webhook is configured correctly!", 0x007ACC);
}
